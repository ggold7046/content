...
At that time, the W3C nearly abandoned HTML in favour of {{Glossary("XHTML")}}, prompting the founding of an independent group called the {{Glossary("WHATWG")}} in 2004. Thanks to WHATWG, work on HTML continued; the two organizations released the first draft of {{Glossary("HTML5")}} in 2008 and an official standard in 2014. The term "HTML5" is just a buzzword referring to modern web technologies that are part of the [HTML Living Standard](https://html.spec.whatwg.org).
...
An HTML file is normally saved with an `.htm` or `.html` extension, is served by a {{Glossary("Server","web server")}}, and can be rendered by any {{Glossary("Browser","Web browser")}}.
