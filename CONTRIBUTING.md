...
- [MDN Markdown Writing Guide][] - Which Markdown features we support and custom extensions we've added.
...
- [GitHub UI](https://docs.github.com/en/repositories/working-with-files/managing-files) -
  This is the easiest way to contribute **small changes**, as described in [Simple changes](#simple_changes).
...
- [`Git`](https://git-scm.com/downloads) - You can use `git` from the command line to interact with GitHub.
  ...
  The [`git` cheat sheet](https://training.github.com/) and the [Using Git](https://docs.github.com/en/get-started) guide are useful resources for beginners and advanced users.
...
See [Creating a pull request](#creating-a-pull-request) for more details on creating and handling pull requests successfully.
...
3. Check for errors by clicking `Show flaws` on each previewed page.
   You might be able to correct flaws by running:
...
3. Commit all of the changed files and push your branch to your fork:
...
- **It can take up to 48 hours** for merged pull requests to have their changes published to [MDN Web Docs](https://developer.mozilla.org).

During reviews, you may be asked to answer questions about your work or to make changes to your suggested edits.
This is a common part of the process of making changes to open source projects.
There are some important rules of etiquette to remember that will help during the review stage.

1. **Check the tests** that are run automatically for pull requests (see [.github/workflows](.github/workflows)).
...
3. **Group logical changes** in each pull request so that it contains a single change or a related set of changes.
   If a pull request becomes too large or contains too many unrelated changes, a reviewer may close it and ask you to submit a new pull request for each set of changes.

4. **Don't reopen pull requests** closed by a reviewer.
...
When contributing to the content, you agree to licence your contributions
